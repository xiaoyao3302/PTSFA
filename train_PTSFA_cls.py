...
parser.add_argument('--loss_function', type=str, default='ISDA', choices=['no_mean', 'use_mean', 'CE'], help='use mean or not in our PTSFA loss or CE')
...
parser.add_argument('--use_src_IDA', type=str2bool, default=False, help='Using the prototype alignment on the source data')
parser.add_argument('--use_trgt_IDA', type=str2bool, default=False, help='Using the prototype alignment on the target data')
...
for epoch in range(args.epochs):
    ...
    for src_data_all, trgt_data_all in zip(src_train_loader, trgt_train_loader):
        ...
        if epoch < args.epoch_warmup:
            ...
            if args.loss_function == 'no_mean':
                PTSFA_loss = criterion_PTSFA(model.cls, src_feature, src_pred, src_label, Lambda, cv_pool)
            elif args.loss_function == 'use_mean':
                PTSFA_loss = criterion_PTSFA(model.cls, src_feature, src_pred, src_label, Lambda, mean_pool, mean_src, cv_pool)
            else:
                # CE loss
            ...
            if args.use_src_IDA:
                src_proto_loss = IDALoss(mean_pool.detach(), src_feature, src_label, args.tao)
            ...
            if args.use_trgt_IDA:
                trgt_proto_loss = IDALoss(mean_pool.detach(), trgt_feature, trgt_pred.max(dim=-1)[1], args.tao)
